...
/**
 * Define a special error for expired refresh token
 */
error.RefreshTokenExpiredError = function (message, body, caller) {
    error.HttpError.call(this,
        405,
        message,
        body || null,
        caller || this.constructor);
};
util.inherits(error.RefreshTokenExpiredError, error.HttpError);
error.RefreshTokenExpiredError.displayName = error.RefreshTokenExpiredError.prototype.name = 'RefreshTokenExpiredError';
...
function useRefreshTokenGrant(next) {
    ...
    this.model.getRefreshToken(token, function (err, refreshToken) {
        ...
        if (!refreshToken || refreshToken.clientId !== self.client.clientId) {
            ...
            return next(new error.RefreshTokenExpiredError('Refresh token has expired'));
        }
